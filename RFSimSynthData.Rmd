  ...
  out <- 
    foreach(i = 1:n_reps, .combine = "rbind", .packages = "foreach", .export = pckgs) %dopar% {
      ...
      foreach (j = 1:length(n_tree), .combine = "c", .export = pckgs) %do% {
        randomRF(regs = regs, y = y, y_perm = y_perm, 
                 n_tree = n_tree, leafsize = leafsize, maxdepth = maxdepth, maxnodes = maxnodes, nodesize = nodesize)
      }
    }
...
powerplot <- function(..., x_names = NA, sampsize = NA, alpha = 0.05){
  data <- list(...)
  ...
  for (i in 1:length(data)){
    tmp <- lapply(data[[i]], function(x, alpha) mean(x < alpha), alpha = alpha)
    data_long[[i]] <- cbind(i, unlist(tmp), x_names)
  }
  return(data_long)
  data_long <- do.call(rbind.data.frame, data_long)
  colnames(data_long) <- c("exp", "power", "n_tree")
  out <- ggplot(data = data_long, aes(x = n_tree, y = power, colour = factor(exp))) + geom_line(linetype = "dashed") + geom_point() + 
    ylim(c(0, 1)) + xlab("Number of trees") + ylab("Power") + scale_color_discrete(name = "Sample size", labels = sampsize)
  return(out)
}
...
powerplot(StabCS3, StabCS6, x_names = n_tree, sampsize = c(20, 40))

powerplot(StabLDA20, StabLDA40, StabLDA80, x_names = n_tree, sampsize = c(20, 40, 80))
```

```{r}
n_tree <- c(50, 100, 250, 500, 1000)
LDA20 <- LDASamp(n_samples = 20, p = 2, effect = 25)
StabLDA20 <- StabExp(n_tree = n_tree, data = LDA20, n_reps = 100, leafsize = 3)
```
...
```{r}
CS12 <- CheckerSamp(n = 12, n_clusts = 3)
StabCS12 <- StabExp(n_tree = n_tree, data = CS12, n_reps = 100, leafsize = 2)
```
...
```{r}
CheckerSim <- function(n_reps, n, n_clusts, n_perms = 1000, n_tree = 250, leafsize = 5, maxdepth = NA, maxnodes = NA, nodesize = NA, ...){
...
y <- sample(c(0, 1), size = 15, replace = TRUE)
tmp <- data.frame(X = X, y = y)
colnames(tmp) <- c("X1", "X2", "y")
set.seed(1)
gt(y ~ 1, y ~ ., data = tmp, permutations = 1000)
...
S_t <- S(X, y)
set.seed(1)
S_p <- c(Inf, replicate(999, S(X, sample(y))))
mean(S_t*(1-1e-14) <= S_p)
...
nestedcv.rf(folds, regs, labs[sample(nrow(labs)), , drop = FALSE])
```
...
```{r}
pval.rf_gt <- function(regs, labs, maxdepth = NA, n_perms = 1000, n_tree = 500, balanced = TRUE, force = FALSE, leafsize = 1, maxnodes = NA, nodesize = NA){
...
pval.rf_gt(LDA20[, !names(LDA20) %in% "y"], LDA20[, "y", drop = FALSE])
```
...
SimExp <- function(data, n_reps, indx = NA, n_perms = 100, ...){
  ...
  res <- matrix(data = NA, nrow = n_reps, ncol = 4) ### initialize output matrix
  colnames(res) <- c("linear.gt", "nestedcv.ridge", "nestedcv.rf", "pval.rf_gt") ### assign names to the output matrix
...
```{r}
labs <- LDA20[, "y", drop = FALSE]
regs <- LDA20[,  !colnames(LDA40) %in% "y"]
