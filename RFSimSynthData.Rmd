...
randomsplit <- function(regs, indx, spltb_reg, nodesize = 1){
  ...
  for (i in 1:length(unq_indx)){
    ...
    if(sum(as.numeric(sub_indx)) < 2*nodesize | all(!spltb_reg[[unq_indx[i]]])) next
    splt <- TRUE
    while(splt){
...
updatesplit <- function(regs, indx, spltb_reg, nodesize){
  out <- indx
  updt_indx <- randomsplit(regs = regs, indx = indx, spltb_reg = spltb_reg, nodesize = nodesize)
  log_indx <- !is.na(updt_indx)
  out[log_indx] <- paste0(indx[log_indx], updt_indx[log_indx])
  return(out)
}

regsplits <- function(regs, indx, nodesize){
  ...
  for (i in 1:length(indx_names)){
    ...
    for (j in 1:ncol(regs)){
      ...
      tmp[[j]] <- (sum(cumtable) - max(cumtable)) >= nodesize
    }
...
regsplits(tmp, rep(c("1", "2"), 10), nodesize = 1)
...
gensplits <- function(regs, labs, maxdepth = NA, maxnodes = NA, nodesize = 1){
  ...
  i <- 1
  spltb_reg <- regsplits(regs, out[i, ], nodesize = nodesize)
  psbl_split <- any(unlist(spltb_reg))
  while (max(nchar(out[i, ])) - 1 < maxdepth &
         length(unique(out[i, ])) < maxnodes &
         !all(table(out[i, ]) < 2*nodesize) &
         psbl_split
         ){
    tmp <- updatesplit(regs = regs, indx = out[i, ], spltb_reg = spltb_reg, nodesize = nodesize)
    
    if(max(nchar(tmp)) - 1 <= maxdepth &
       length(unique(tmp)) <= maxnodes &
       all(table(tmp) >= nodesize)){
...
gensplits(tmp, sample(c(0, 1), size = 20, replace = TRUE), nodesize = 2)
