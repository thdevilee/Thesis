...
```{r}
png("C:/Users/Thomas/Documents/Thesis/RF/Plots/RF_StabLDA.png")
powerplot(StabLDA, x_names = n_tree, xlab = "Number of trees", sampsize = c(20))
dev.off()
```

```{r}
png("C:/Users/Thomas/Documents/Thesis/RF/Plots/RF_StabCS.png")
powerplot(StabCS, x_names = c(n_tree, 50000), xlab = "Number of trees", sampsize = c(27))
dev.off()
```
...
LDASim <- function(sampsize, n_reps, effect, p, n_perms = 1000, ...){
  ...
  for (i in 1:nrow(params)){
    ...
    res <- foreach (j = 1:n_reps, .combine = "rbind", .export = pckgs, .packages = c("doParallel", "randomForest", "globaltest")) %dopar% {
      ...
      SimExp(n_reps = 1, data = data, indx = NA, n_perms = n_perms, ...)
    }
...
png("C:/Users/Thomas/Documents/Thesis/RF/Plots/RF_LDASimDim.png")
...
png("C:/Users/Thomas/Documents/Thesis/RF/Plots/RF_LDASimeffect.png")
...
png("C:/Users/Thomas/Documents/Thesis/RF/Plots/RF_CheckerboardSimnclusts.png")
...
png("C:/Users/Thomas/Documents/Thesis/RF/Plots/RF_CheckerboardSimN.png")
...
gentrees <- function(regs, n_tree, maxdepth = NA, maxnodes = NA, nodesize = NA, leafsize = 1){
  pckgs <- as.vector(lsf.str(.GlobalEnv))
  out <- foreach(j = 1:n_tree, .combine = "cbind", .export = pckgs) %dopar% { ################## change back
    splts <- gensplits(regs = regs, maxdepth = maxdepth, maxnodes = maxnodes, nodesize = nodesize, leafsize = leafsize)
    sapply(splts, FUN = function(x) as.numeric(substr(x, nchar(x), nchar(x)) == 2))
  }
  out <- as.matrix(out)
...
all.tests <- function(regs, labs, indx = NA, n_perms = 1000, ...){
  ...
  pckgs <- as.vector(lsf.str(.GlobalEnv))
  names(res) <- c("linear.gt", "nestedcv.ridge", "fixedcv.rf", "fixed.rf_gt", "pval.rf_gt", "overall.Sgt") ### assign names to the output matrix
  ...
  perm_mat <- foreach(j = 1:(n_perms-1), .combine = "cbind", .export = pckgs) %dopar% { ### for n_perms replications
    ...
    perm_folds_ridge <- create_folds(n_folds = 5, labs = labs, ...) ### compute folds
    perm_folds_rf <- create_folds(n_folds = 4, labs = labs, ...) ### compute folds
    perm_ridge <- nestedcv.ridge(folds = perm_folds_ridge, regs = perm_regs, labs = labs) ### compute (permuted) test statistic for the nested ridge
    perm_rf <- fixedcv.rf(folds = perm_folds_rf, regs = perm_regs, labs = labs) ### compute (permuted) test statistic for the nested knn
    matrix(c(perm_ridge, perm_rf), nrow = 2) ### return permuted test statistics for both tests
  }
...
labs <- impact[c(1:20, 11003:11022), c("mort"), drop = FALSE]
names(labs) <- "y"
