...
```{r}
set.seed(1979418)
LDAs_depth <- PowerExp(mthd = "LDA", sampsize = 40, effect = 25, p = 2, n_reps = 100, maxdepth  = c(seq(1, 10), 15, 10, 20, 40))
```

```{r}
set.seed(1979418)
CS_depth <- PowerExp(mthd = "CS", n_obs_clusts = 20, n_clusts = 3, n_reps = 100, maxdepth = c(seq(1, 10), 15, 20, 40, 80, 180))
```
...
overall.Sgt <- function(regs, labs, n_tree = 250, maxdepth = NA, n_perms = 1000, leafsize = 1, maxnodes = NA, nodesize = NA){
  ...
  y_perm <- sapply(1:(n_perms - 1), function(x, y) y[sample(nrow(y)), ,drop = FALSE], y = y)
  X <- gentrees(regs = regs, n_tree = 1, leafsize = leafsize, maxdepth = ceiling(runif(1, min = 0, max = n - 1)), maxnodes = maxnodes, nodesize = nodesize)
  for(i in 1:(n_tree-1)){
    X <- cbind(X, gentrees(regs = regs, n_tree = 1, leafsize = leafsize, maxdepth = ceiling(runif(1, min = 0, max = n - 1)), maxnodes = maxnodes, nodesize = nodesize))
  }
  S_t <- S(X, y)
  S_p <- c(Inf, S(X, y_perm))
  out <- mean(S_t*(1-1e-14) <= S_p)
