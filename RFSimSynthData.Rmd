...
StabCS <- StabExp(n_tree = c(50, 100, 250, 500, 1000, 2500, 5000, 10000, 25000), data = CSdata, n_reps = 100, leafsize = 2)
...
```{r}
fixed.rf_gt <- function(regs, labs, leafsize = 5, n_perms = 1000, n_tree = 5000, maxdepth = NA, maxnodes = NA, nodesize = NA){
...
```{r}
pval.rf_gt <- function(regs, labs, maxdepth = NA, n_perms = 1000, n_tree = 5000, balanced = TRUE, force = FALSE, leafsize = 1, maxnodes = NA, nodesize = NA){
  ...
  for (i in 1:length(maxdepth)){
    p_vals[i] <- randomRF(regs = regs[indx_train, ], y = y_train, y_perm = y_perm_train, n_tree = 500, leafsize = leafsize, 
...
```{r}
overall.Sgt <- function(regs, labs, n_tree = 5000, maxdepth = NA, n_perms = 1000, leafsize = 1, maxnodes = NA, nodesize = NA){
  pckgs <- as.vector(lsf.str(.GlobalEnv))
  n <- nrow(regs)
  y <- matrix(as.numeric(labs[["y"]]))
  y_perm <- sapply(1:(n_perms - 1), function(x, y) y[sample(nrow(y)), ,drop = FALSE], y = y)
...
```{r}
LDAS_effect <- LDASim(sampsize = 40, n_reps = 100, effect = c(25, 20, 10, 1, 0.05), p = 2)
```
