...
Stat <- apply(boom, 2, function(x, y) S(X = as.matrix(x), y = y), y = matrix(CSdata[, "y"]))
```
...
```{r}
fixed.rf_gt <- function(regs, labs, leafsize = 5, n_perms = 1000, n_tree = 250, maxdepth = NA, maxnodes = NA, nodesize = NA){
...
```{r}
regs <- LDAdata[, !names(LDAdata) %in% "y"]
labs <- LDAdata[, "y", drop = FALSE]
