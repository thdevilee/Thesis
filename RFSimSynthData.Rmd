...
StabExp <- function(n_tree, data, n_reps = 100, n_perms = 1000, leafsize = 5, maxdepth = NA, maxnodes = NA, nodesize = NA){
  ...
  out <- 
    foreach(i = 1:n_reps, .combine = "rbind", .packages = "foreach", .export = pckgs) %dopar% {
      ...
      foreach (j = 1:length(n_tree), .combine = "c", .export = pckgs) %dopar% {
...
SimExp <- function(data, n_reps, pckgs, indx = NA, n_perms = 100, ...){
  ...
  colnames(res) <- c("linear.gt", "nestedcv.ridge", "nestedcv.rf", "rf.gt") ### assign names to the output matrix
  ...
  for(i in 1:max(indx)){ ### for each folds
    ...
    rfcv_stat <- nestedcv.rf(folds = folds, regs = sub_regs, labs = sub_labs) ### compute test statistic for the nested knn
    rf_X <- gentrees()
    rfgt_stat <- 
    ...
    perm_mat <- foreach(j = 1:n_perms, .combine = "cbind", .export = pckgs) %dopar% { ### for n_perms replications
      ...
      perm_rfcv <- nestedcv.rf(folds = perm_folds, regs = perm_regs, labs = sub_labs) ### compute (permuted) test statistic for the nested knn
      perm_rfgt
    ...
    res[i, 3] <- mean(rfcv_stat*(1-1e-14) <= c(Inf, perm_mat[2, , drop = TRUE])) ### compute emperical p-value for nested knn
  }
  return(res)
}
```
