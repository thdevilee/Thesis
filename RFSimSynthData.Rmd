...
randomRF(tmp, data.frame(sample(c(0, 1), replace = TRUE, size = 20)), leafsize = 3, n_tree = 100, n_perms = 1000)

LDASim <- function(sampsize, n_reps, n_tree, n_perms, maxdepth = NA, maxnodes = NA, nodesize = NA, leafsize = 1){
  indx_args <- c(length(sampsize) > 1, length(effect) > 1, length(sampsize) > 1)
...
reps <- 100
linear_data <- LDASamp(n_samples = 20, p = 2, effect = 25)

res <- vector(mode = "list", length = length(n_tree))
for (i in 1:length(n_tree)){
  res[[i]] <- replicate(reps, expr = 
                          randomRF(linear_data[, c("x1", "x2")], linear_data[, c("y")], n_tree = n_tree[i], n_perms = n_perms, leafsize = leafsize)
                        )
}
```
