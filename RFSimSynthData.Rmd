...
```{r}
powerplot(StabCS3, StabCS6, x_names = n_tree, xlab = "Number of trees", sampsize = c(20, 40))
```
  ...
  if(mthd == "LDA"){
    data <- lapply(1:n_reps, FUN = function(x, sampsize, p, effect) LDASamp(n_samples = sampsize, p = p,
     ...
     data <- lapply(1:n_reps, FUN = function(x, n, n_clusts) CheckerSamp(n = n, n_clusts = n_clusts),
                   n = samp_ops[["n_obs_clust"]], n_clusts = samp_ops[["n_clusts"]])
...
```{r}
set.seed(1979418)
LDAs_depth <- PowerExp(mthd = "LDA", sampsize = 40, effect = 25, p = 2, n_reps = 100, maxdepth  = c(1, 2, 5, 10, 20, 40))
```

```{r}
set.seed(1979418)
CS_depth <- PowerExp(mthd = "CS", n_obs_clusts = 20, n_clusts = 3, n_reps = 100, maxdepth = c(1, 2, 5, 10, 20, 40, 80, 180))
```
