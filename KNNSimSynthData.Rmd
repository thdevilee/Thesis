...
InvPropWeight <- function(P, y){
  ...
  return(c(mu.num, mu.den, var.den, var.num, cov.term, VarS))
  weight <- 1/VarS
...
LDASim <- function(sampsize, n_reps, effect, p, pckgs, ...){
  ...
  out <- matrix(ncol = 7, nrow = nrow(params))
...
tmp <- LDASim(sampsize = 40, n_reps = 100, effect = 25, p = c(2, 15, 25, 50, 100), pckgs = pckgs)
```
