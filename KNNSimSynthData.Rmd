...
overall.gt <- function(regs, labs, ord_mat, invweighted = FALSE){
...
prop.knn_gt <- function(regs, labs, ord_mat){
...
overallordinal.gt <- function(regs, labs, ord_mat){
...
SimExp <- function(data, n_reps, pckgs, indx = NA, n_perms = 1000, ...){
  ...
  for(i in 1:max(indx)){ ### for each folds
    ...
    res[i, 4] <- prop.knn_gt(regs = sub_regs, labs = sub_labs, ord_mat = ord_mat) ### compute p-value for the global test on the partition matrix based on the label proportion
    ...
    res[i, c(6, 7)] <- overall.gt(regs = sub_regs, labs = sub_labs, ord_mat = ord_mat, invweighted = TRUE) ### compute unweighted and inversely weighted overall gloval test
    res[i, 8] <- overallordinal.gt(regs = sub_regs, labs = sub_labs, ord_mat = ord_mat) ### compute unweighted and inversely weighted overall gloval test
