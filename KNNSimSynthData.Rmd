...
overall.gt <- function(regs, labs, invweighted = FALSE){
  ...
  if (invweighted){ ### if inverse weights
    ...
    weights <- round(weights/sum(weights)*100, 6)
  }
...
pval.knn_gt <- function(regs, labs, balanced = TRUE, force = FALSE){
  ...
  neighbours <- round(seq(2, n_train, length.out = round(sqrt(n_train))))
...
tmp <- CheckerSim(n_reps = 100, n = 5, n_clusts = 3, pckgs = pckgs, force = TRUE)
```
...
LDASim <- function(sampsize, n_reps, effect, p, pckgs, ...){
  if(as.numeric(length(p) > 1) + as.numeric(length(effect) > 1) + as.numeric(length(sampsize) > 1) > 1) stop("Only one parameter can have multiple values") ### only allow for one parameter to take on multiple values
  params <- cbind(p, effect, n_samples) ### parameter space
  output <- list(nrow(params)) ### initialize empty list for all elements of p
  data <- as_tibble(LDASamp(n_samples = sampsize * n_reps, p = p, effect = effect))
  out <- SimExp(n_reps = n_reps, data = data, pckgs = pckgs, ...)
  out <- EmpericalPower(out)
  return(out)
}
LDASim(sampsize = 40, n_reps = 100, effect = 25, p = 20, pckgs = pckgs)

tmp <- matrix(ncol = 7, nrow = 5)
ps <- c(2, 20, 40, 60, 80, 100)
for (i in 1:length(ps)){
 tmp[i, ] <- LDASim(sampsize = 40, n_reps = 100, effect = 25, p = ps[i], pckgs = pckgs)
}
```
