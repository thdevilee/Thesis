...
pval.knn_gt <- function(regs, labs){
  ...
  folds <- create_folds(n_folds = 2, labs = labs) ### create folds such that the groups are balanced
  ...
  ord_mat <- knn_mat(dist_mat) ### compute ordering mat
  p_vals <- numeric(n_train) ### initialize vector for p-values
  p_vals[1] <- 1 ### skip p-value for partition matrix which corresponds to k = 1
  for (j in 2:n_train){ ### for all 2, ..., k neighbours
...
```{r}
pckgs <- c("create_folds", "linear.gt", "nestedcv.ridge", "nestedcv.knn", "prop.knn_gt", "pval.knn_gt", "overall.gt", "rename_folds", "knn_mat", "is.int", "gt")
SimExp <- function(n_reps, n_perms, data, pckgs){
  ...
  names(res) <- c("linear.gt", "nestedcv.ridge", "nestedcv.knn", "prop.knn_gt", "pval.knn_gt", "overall.gt")
...
tmp1 <- SimExp(n_reps = 100, n_perms = 100, data = data[["linear"]][c(1:2000, 50001:52000), ], pckgs = pckgs)
```
