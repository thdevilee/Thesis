...
overall.gt <- function(labs, ord_mat, invweighted = FALSE){
...
```{r}
data <- LDASamp(40, 2, 25)
regs <- data[, !names(data) %in% "y"]
labs <- data[, "y", drop = FALSE]
...
overall.gt(labs, knn_mat(comp_dist(regs)), n_perms = 1000, invweighted = TRUE)
```
...
SimExp <- function(data, n_reps, pckgs, indx = NA, n_perms = 1000, ...){
  ...
  for(i in 1:max(indx)){ ### for each folds
    ...
    res[i, c(6, 7)] <- overall.gt(labs = sub_labs, ord_mat = ord_mat, invweighted = TRUE) ### compute unweighted and inversely weighted overall gloval test
