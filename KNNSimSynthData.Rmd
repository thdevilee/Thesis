...
all.tests <- function(regs, labs, pckgs, n_perms = 100, ...){
  ...
  res[4] <- prop.knn_gt(regs = regs, labs = labs, ord_mat = ord_mat) ### compute p-value for the global test on the partition matrix based on the label proportion
  ...
  res[c(6, 7)] <- overall.gt(regs = regs, labs = labs, ord_mat = ord_mat, invweighted = TRUE) ### compute unweighted and inversely weighted overall gloval test
  res[8] <- overallordinal.gt(regs = regs, labs = labs, ord_mat = ord_mat) ### compute unweighted and inversely weighted overall gloval test
...
all.tests(regs = regs, labs = labs, n_perms = 10)
```
...
names(impact)[names(impact) == "mort"] <- "y"
impact <- as_tibble(impact)[c(1:20, 11003:11022), ]
regs <- impact[c(1:20, 11003:11022), !names(impact) %in% c("mort")]
labs <- impact[c(1:20, 11003:11022), c("y")]
indx <- sample(c(TRUE, FALSE), size = 40, replace = TRUE)

regs <- impact[c(1:300, 10001:10300), !names(impact) %in% c("mort")]
labs <- impact[c(1:300, 10001:10300), c("y")]

all.tests(regs = regs, labs = labs, n_perms = 10)

indx <- sample(c(TRUE, FALSE), size = 40, replace = TRUE)
knn3(formula = y ~ ., data = impact)
```
