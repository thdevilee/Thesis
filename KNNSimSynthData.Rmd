...
CheckerSim <- function(n_reps, n, n_clusts, n_perms = 1000, ...){
  ...
  for (i in 1:nrow(params)){
    cat("Start element", i, "\n")
    res <- foreach (j = 1:n_reps, .combine = "rbind", .export = pckgs, .packages = c("doParallel", "globaltest", "tidyverse")) %dopar% {
      data <- as_tibble(CheckerSamp(n = params[[i, "n"]], n_clusts = params[[i, "n_clusts"]]))
      SimExp(n_reps = 1, data = data, indx = NA, n_perms = n_perms, pckgs = pckgs, ...)
    }
...
png("C:/Users/Thomas/Documents/Thesis/KNN/Plots/KNN_CheckerboardSimnclusts.png")
exprplot(data = CBS_nclusts, x_names = c(3, 5), xlab = "Number of clusters")
dev.off()
```

```{r}
CBS_n <- CheckerSim(n_reps = 100, n = c(2, 5, 10), n_clusts = 3, n_perms = 1000, force = TRUE)
png("C:/Users/Thomas/Documents/Thesis/KNN/Plots/KNN_CheckerboardSimN.png")
exprplot(data = CBS_n, x_names = c(2, 5, 10), xlab = "Observations per tile")
dev.off()
```
...
LDASim <- function(sampsize, n_reps, effect, p, n_perms = 1000, ...){
  ...
  for (i in 1:nrow(params)){
    cat("Start element", i, "\n")
    res <- foreach (j = 1:n_reps, .combine = "rbind", .export = pckgs, .packages = c("doParallel", "globaltest", "tidyverse")) %dopar% {
      data <- as_tibble(LDASamp(n_samples = params[i, "sampsize"], p = params[i, "p"], effect = params[i, "effect"]))
      SimExp(n_reps = 1, data = data, indx = NA, n_perms = n_perms, pckgs = pckgs, ...)
    }
    out[[i]] <- EmpericalPower(res)
    end <- Sys.time()
...
png("C:/Users/Thomas/Documents/Thesis/KNN/Plots/KNN_LDASimDim.png")
exprplot(data = LDAS_p, x_names = c(2, 10, 25, 50, 100), xlab = "Dimensionality")
dev.off()
```
...
png("C:/Users/Thomas/Documents/Thesis/KNN/Plots/KNN_LDASimeffect.png")
exprplot(data = LDAS_effect, x_names = c(5, 4, 3, 2, 1), xlab = "Effect size")
dev.off()
```
...
```{r}
all.tests <- function(regs, labs, n_perms = 1000, ...){
  res <- rep(NA, 8)
  pckgs <- as.vector(lsf.str(.GlobalEnv))
  ...
  ridge_stat <- nestedcv.ridge(folds = folds, regs = regs_alt, labs = labs) ### compute test statistic for the nested ridge
  knn_stat <- nestedcv.knn(folds = folds, regs = regs, labs = labs) ### compute test statistic for the nested knn
  res[4] <- prop.knn_gt(labs = labs, ord_mat = ord_mat) ### compute p-value for the global test on the partition matrix based on the label proportion
  ...
  res[c(6, 7)] <- overall.gt(labs = labs, ord_mat = ord_mat, invweighted = TRUE, n_perms = 1000) ### compute unweighted and inversely weighted overall gloval test
  res[8] <- overallordinal.gt(labs = labs, ord_mat = ord_mat) ### compute unweighted and inversely weighted overall gloval test
  ...
  perm_mat <- foreach(j = 1:(n_perms-1), .combine = "cbind", .export = pckgs) %dopar% { ### for n_perms replications
   perm_indx <- sample(nrow(labs)) ### permutation index
    ...
    perm_regs_alt <- regs_alt[perm_indx, ]
    perm_folds <- create_folds(n_folds = 5, labs = labs, ...) ### compute folds
    perm_ridge <- nestedcv.ridge(folds = perm_folds, regs = perm_regs_alt, labs = labs) ### compute (permuted) test statistic for the nested ridge
  ...
  res[3] <- mean(knn_stat*(1-1e-14) <= c(Inf, perm_mat[2, , drop = TRUE])) ### compute emperical p-value for nested knn
  names(res) <- c("linear.gt", "nestedcv.ridge", "nestedcv.knn", "prop.knn_gt", "pval.knn_gt", "overall.gt", "invoverall.gt", "overallordinal.gt") ### assign names to the output
  return(res)
}
all.tests(regs = regs, labs = labs, n_perms = 1000, force = FALSE)
```
...
set.seed(89)
n <- 1000
prop <- mean(impact$mort)
n_1 <- round(n*prop)
n_0 <- n - n_1
impact <- as_tibble(impact)
indx <- split(seq(1, nrow(impact)), impact$mort)
indx <- c(sample(indx[[1]], n_0), sample(indx[[2]], n_1))
regs <- impact[indx, !names(impact) %in% c("mort")]
labs <- impact[indx, c("mort")]
names(labs) <- "y"


all.tests(regs = regs, labs = labs, n_perms = 1000, force = TRUE)
```
...
impact <- as_tibble(impact)
regs <- impact[c(1:40, 10983:11022), !names(impact) %in% c("mort")]
labs <- impact[c(1:40, 10983:11022), c("mort")]
names(labs) <- "y"

indx <- sample(c(rep(TRUE, 200), rep(FALSE, nrow(impact) - 200)))
regs <- impact[indx, !names(impact) %in% c("mort")]
labs <- impact[indx, c("mort")]
names(labs) <- "y"

all.tests(regs = regs, labs = labs, n_perms = 1000)
data <- data.frame(regs, labs)
indx <- sample(c(TRUE, FALSE), size = 40, replace = TRUE)
tmp <- knn3(formula = y ~ ., data = dat)
predict(tmp, dat, type = "class")

folds <- create_folds(5, labs)
```
